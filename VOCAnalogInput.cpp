...
VOCAnalogInput::VOCAnalogInput(int Pin, String configFile) {
...
bool VOCAnalogInput::begin() {
	values.resize(1);
	// Set description
	Description.type = "analog input";
	Description.name = "VOC Analog Input";
	Description.id = 0;
...
String VOCAnalogInput::getConfig() {
...
/// @return True on success
bool VOCAnalogInput::setConfig(String config, bool save) {
	// Allocate the JSON document
  	JsonDocument doc;
...
/// @return True on success
bool VOCAnalogInput::takeMeasurement() {
...
/// @return True on success
bool VOCAnalogInput::configureInput() {
...
int VOCAnalogInput::getAnalogValue(bool average) {
	if (!average) {
...
int VOCAnalogInput::analogToMV(int value) {
